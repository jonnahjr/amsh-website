...
export default function ServiceCategoriesAdmin() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            alert('Service node synchronization failed.');
        }
    };

    const handleDelete = async (id: string) => {
        if (!confirm('Purge this service category? This will affect public orientation pathways.')) return;
        try {
            await serviceCategoriesAPI.delete(id);
            fetchCategories();
        } catch (error) {
            console.error('Delete category error:', error);
            alert('Delete protocol rejected.');
        }
    };
    ...
    const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            alert('Visual asset link failed.');
        } finally {
            setUploading(null);
        }
    };
    ...
    return (
        <div className="space-y-12 animate-in fade-in duration-700 pb-20 max-w-[1600px] mx-auto">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-10 bg-white p-10 lg:p-14 rounded-[3.5rem] border border-slate-200/60 shadow-sm relative overflow-hidden group">
                ...
                <div className="relative z-10 flex flex-col md:flex-row md:items-center gap-8">
                    ...
                    <div>
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <span className="text-[10px] text-slate-400 font-black uppercase tracking-[0.3em]">Operational Service Hub</span>
                        </div>
                        <h2 className="text-4xl lg:text-5xl font-jakarta font-black text-slate-900 tracking-tight leading-none">Service Taxonomy</h2>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-4 relative z-10">
                    ...
                    >
                        ...
                        <span>Initialize Service Node</span>
                    </button>
                </div>
            </div>
            ...
            <div className="relative max-w-xl group">
                ...
                <input
                    type="text"
                    placeholder="Scan service groups by nomenclature..."
                    value={searchTerm}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                {loading ? (
                    ...
                    <div className="col-span-full bg-white py-40 rounded-[4rem] border border-slate-200/60 shadow-sm text-center">
                        ...
                        <h3 className="text-3xl font-jakarta font-black text-slate-900 mb-2">Null Service Matrix</h3>
                        <p className="text-slate-400 font-bold uppercase tracking-[0.3em] text-[10px]">No service categories match your currently active telemetry scan.</p>
                    </div>
                ) : (
                    categories
                        ...
                        .map((category, idx) => (
                            <div key={category.id} className="bg-white rounded-[4rem] border border-slate-200/60 shadow-sm hover:shadow-2xl hover:border-primary/20 transition-all duration-700 group overflow-hidden flex flex-col animate-in slide-in-from-bottom-12" style={{ animationDelay: `${idx * 100}ms` }}>
                                <div className="h-64 relative overflow-hidden">
                                    ...
                                    <div className="absolute bottom-8 left-8 right-8 text-white z-10">
                                        <div className="flex items-center gap-3 mb-2">
                                            ...
                                            <span className="text-[10px] font-black uppercase tracking-[0.4em] text-primary">Service Hub Node</span>
                                        </div>
                                ...
                                <div className="p-10 flex-1 flex flex-col bg-white">
                                    <p className="text-slate-500 text-sm leading-relaxed font-medium line-clamp-3 mb-10 flex-1">{category.description || 'No descriptive protocol provided for this service taxonomy node.'}</p>
                                    ...
                                    <div className="flex items-center justify-between mt-auto pt-8 border-t border-slate-50">
                                        ...
                                        <div className="flex flex-col items-end gap-1">
                                            <span className="text-[9px] font-black text-slate-300 uppercase tracking-widest">Linked Entities</span>
                                            <span className="text-xs font-black text-primary">{category.deptSlugs ? category.deptSlugs.split(';').length : 0} Departments</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className="bg-white w-full max-w-2xl rounded-[4rem] shadow-[0_40px_100px_rgba(0,0,0,0.5)] overflow-hidden animate-in zoom-in-95 duration-300 border border-white/20">
                            ...
                            <div className="flex items-center gap-6">
                                ...
                                <div>
                                    ...
                                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em]">
                                        {editingCategory ? 'Node Configuration Update' : 'New Service Gateway Insertion'}
                                    </p>
                                </div>
                            </div>
                        ...
                        <form onSubmit={handleSubmit} className="p-12 space-y-10 max-h-[75vh] overflow-y-auto no-scrollbar">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                ...
                                <div className="space-y-3">
                                    <label className="text-[10px] font-black text-primary uppercase tracking-[0.3em] ml-2">Glyph (Emoji)</label>
                                    <input
                                        type="text"
                                        value={formData.icon}
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="space-y-3">
                                    <label className="text-[10px] font-black text-primary uppercase tracking-[0.3em] ml-2">Global Route Slug</label>
                                    <input
                                        type="text"
                                        required
                                        value={formData.slug}
                                ...
                                <div className="space-y-3">
                                    <label className="text-[10px] font-black text-primary uppercase tracking-[0.3em] ml-2">Telemetry Priority</label>
                                    <input
                                        type="number"
                            ...
                            <div className="space-y-3">
                                <label className="text-[10px] font-black text-primary uppercase tracking-[0.3em] ml-2">Service Brief (Protocol)</label>
                                <textarea
                                    required
                                    rows={3}
                                    ...
                                    placeholder="Define the scope of this service category..."
                                />
                            </div>

                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="space-y-3">
                                    <label className="text-[10px] font-black text-primary uppercase tracking-[0.3em] ml-2">Background Schema (CSS)</label>
                                    <input
                                        type="text"
                                        value={formData.gradient}
                                        onChange={(e) => setFormData({ ...formData, gradient: e.target.value })}
                                        className="w-full px-8 py-5 bg-slate-50 border-0 rounded-3xl text-sm font-black text-slate-900 focus:ring-[12px] focus:ring-primary/5 transition-all shadow-inner"
                                        placeholder="from-slate-900 to-primary"
                                    />
                                </div>
                                <div className="space-y-3">
                                    <label className="text-[10px] font-black text-primary uppercase tracking-[0.3em] ml-2">Secondary Accent Signal</label>
                                    <input
                                        type="text"
                                        value={formData.accentColor}
                                        onChange={(e) => setFormData({ ...formData, accentColor: e.target.value })}
                                        className="w-full px-8 py-5 bg-slate-50 border-0 rounded-3xl text-sm font-black text-slate-900 focus:ring-[12px] focus:ring-primary/5 transition-all shadow-inner"
                                        placeholder="bg-primary-dark"
                                    />
                                </div>
                            </div>

                            <div className="space-y-3">
                                <label className="text-[10px] font-black text-primary uppercase tracking-[0.3em] ml-2">Associated Department Tokens (Semicolons)</label>
                                <input
                                    type="text"
                                    value={formData.deptSlugs}
                                    onChange={(e) => setFormData({ ...formData, deptSlugs: e.target.value })}
                                    className="w-full px-8 py-5 bg-slate-50 border-0 rounded-3xl text-sm font-black text-slate-900 focus:ring-[12px] focus:ring-primary/5 transition-all shadow-inner placeholder:text-slate-300"
                                    placeholder="oncology;cardiology;internal-medicine"
                                />
                                <p className="text-[9px] font-bold text-slate-300 ml-4 tracking-widest uppercase italic">Maps this category to specific institutional entities.</p>
                            </div>

                            <div className="space-y-8 pt-6 border-t border-slate-50">
                                <div className="flex items-center justify-between px-2">
                                    <label className="text-[10px] font-black text-primary uppercase tracking-[0.3em]">Gateway Backdrop</label>
                                    <div className="relative">
                                        <input
                                            ...
                                            accept="image/*"
                                        />
                                        <span className={`flex items-center gap-3 px-6 py-3 bg-slate-950 text-white rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${uploading === 'image' ? 'opacity-50' : 'hover:bg-primary active:scale-95 shadow-xl shadow-slate-900/40'}`}>
                                            {uploading === 'image' ? <ArrowPathIcon className="w-4 h-4 animate-spin" /> : <CloudArrowUpIcon className="w-4 h-4" />}
                                            {uploading === 'image' ? 'UPLOADING...' : 'TRANSMIT PHOTO'}
                                        </span>
                                    </div>
                                </div>
                                {formData.image && (
                            ...
                            <div className="flex flex-col md:flex-row items-center gap-10 pt-10 border-t border-slate-50">
                                <label className="flex items-center gap-5 cursor-pointer group w-full md:w-auto">
                                    ...
                                    <span className="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] group-hover:text-primary transition-colors">Tactical Visibility</span>
                                </label>
                                ...
                                >
                                    {editingCategory ? 'SYNCHRONIZE PROTOCOL' : 'DEPLOY SERVICE NODE'}
                                </button>
                            </div>
                        </form>
                    </div>
