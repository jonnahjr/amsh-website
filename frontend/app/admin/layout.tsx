...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
    ...
    return (
        <div className="min-h-screen bg-[#F8FAFC] flex selection:bg-indigo-600 selection:text-white font-[Outfit,sans-serif]">
            {/* Sidebar Overlay for Mobile */}
            {sidebarOpen && (
                <div
                    className="fixed inset-0 bg-blue-950/20 backdrop-blur-sm z-40 lg:hidden"
                    onClick={() => setSidebarOpen(false)}
                />
            )}

            {/* Sidebar */}
            <aside className={`fixed inset-y-0 left-0 bg-[#0B1224] z-50 transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] overflow-hidden flex flex-col shadow-[25px_0_60px_rgba(0,0,0,0.1)] ${sidebarOpen ? 'w-[320px]' : 'w-[100px]'}`}>
                ...
                <div className="h-28 flex items-center px-8 border-b border-white/5 relative bg-gradient-to-b from-white/[0.03] to-transparent">
                    <Link href="/" className="flex items-center gap-5 group">
                        <div className="w-14 h-14 rounded-full bg-white flex items-center justify-center shadow-2xl shadow-blue-500/10 border border-white/10 group-hover:scale-110 group-hover:rotate-6 transition-all duration-700 relative overflow-hidden">
                            <div className="absolute inset-0 bg-gradient-to-tr from-transparent via-white/20 to-transparent translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000" />
                            <img src="/images/logo.png" alt="EMSH Logo" className="w-full h-full object-cover rounded-full" />
                        </div>
                        {sidebarOpen && (
                            <div className="animate-in fade-in slide-in-from-left-4 duration-700">
                                <div className="font-black text-white text-xl leading-none tracking-tight uppercase">AMSH</div>
                                <div className="text-[10px] text-blue-400/60 uppercase font-black tracking-[0.3em] mt-2 flex items-center gap-2">
                                    <span className="w-2 h-[1px] bg-blue-400/30" />
                                    COMMAND
                                </div>
                            </div>
                        )}
                        ...
                        {sidebarOpen && (
                            <div className="pt-4 border-t border-white/5 flex items-center gap-4">
                                <div className="relative group/avatar">
                                    <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-600 to-blue-700 flex items-center justify-center text-white text-xs font-black shadow-lg group-hover/avatar:rotate-6 transition-transform">
                                        {user.name.charAt(0)}
                                    </div>
                                    <div className="absolute -top-1 -right-1 w-3 h-3 bg-emerald-500 border-2 border-[#0B1224] rounded-full" />
                                </div>
                                <div className="flex-1 min-w-0">
                                    <p className="text-[11px] font-black text-white truncate uppercase tracking-tight">{user.name}</p>
                                    <p className="text-[9px] font-bold text-blue-400 opacity-60 uppercase tracking-widest">{user.role}</p>
                                </div>
                            </div>
                        )}
                    </div>
            ...
            <main className={`flex-1 transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] ${sidebarOpen ? 'pl-[320px]' : 'pl-[100px]'}`}>
                ...
                <header className="h-28 bg-white/40 backdrop-blur-3xl border-b border-gray-200/50 flex items-center justify-between px-12 sticky top-0 z-40">
                    <div className="flex items-center gap-8">
                        <button
                            onClick={() => setSidebarOpen(!sidebarOpen)}
                            className="w-14 h-14 flex items-center justify-center bg-white border border-gray-100 hover:border-indigo-100 hover:shadow-2xl hover:shadow-indigo-500/10 rounded-2xl text-gray-400 hover:text-indigo-600 transition-all duration-500 group"
                        >
                        ...
                        <div className="hidden md:block">
                            <h1 className="text-gray-950 font-black text-2xl tracking-tighter">
                            ...
                            <div className="text-[9px] text-gray-400 font-bold uppercase tracking-[0.2em] flex items-center gap-2 mt-1">
                                <span className="text-blue-900/40">Core Console</span>
                    ...
                    <div className="flex items-center gap-10">
                        <div className="hidden xl:flex items-center gap-5 px-6 py-3 bg-indigo-50/50 border border-indigo-100/30 rounded-2xl">
                            <div className="relative flex h-2.5 w-2.5">
                                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-indigo-400 opacity-75" />
                                <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-indigo-600" />
                            </div>
                            <span className="text-[11px] font-black text-indigo-900 uppercase tracking-[0.2em]">Console Live</span>
                        </div>

                        <div className="flex items-center gap-6 border-l border-gray-200/50 pl-10">
                            <div className="text-right hidden sm:block">
                                <div className="text-sm font-black text-gray-950 uppercase tracking-tight leading-none">{user.name}</div>
                                <div className="text-[10px] font-black text-indigo-600 uppercase tracking-[0.2em] mt-1.5 opacity-80">{user.role}</div>
                            </div>
                            <div className="relative group cursor-pointer">
                                <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-indigo-600 to-blue-700 flex items-center justify-center text-white font-black text-xl shadow-2xl shadow-indigo-600/20 ring-4 ring-white transition-transform duration-500 group-hover:scale-105">
                                    {user.name.charAt(0)}
                                </div>
            ...
            {item.href ? (
                <Link
                    ...
                    className={`flex items-center gap-5 px-6 py-4 rounded-2xl transition-all duration-500 group relative overflow-hidden ${active
                        ? 'bg-gradient-to-r from-indigo-600 to-blue-600 text-white shadow-2xl shadow-indigo-600/30 translate-x-1'
                        : 'text-white/40 hover:text-white hover:bg-white/[0.04]'
                        }`}
                    title={!sidebarOpen ? item.label : ''}
                >
                    <item.icon className={`w-[22px] h-[22px] flex-shrink-0 transition-all duration-500 ${active ? 'text-white scale-110' : 'group-hover:scale-110 group-hover:text-blue-400'}`} />
                ...
                >
                    <item.icon className={`w-[22px] h-[22px] flex-shrink-0 transition-all duration-500 ${active && !isOpen ? 'text-white scale-110' : 'group-hover:scale-110 group-hover:text-blue-400'}`} />
                    {sidebarOpen && <span className="font-black text-[12px] tracking-widest uppercase">{item.label}</span>}
                    {sidebarOpen && (
                        <div className={`ml-auto transition-transform duration-500 ${isOpen ? 'rotate-180 text-blue-400' : 'text-white/20'}`}>
            ...
            {hasChildren && sidebarOpen && isOpen && (
                <div className="mt-4 ml-10 pl-6 border-l border-white/10 space-y-3 animate-in fade-in slide-in-from-top-2 duration-500 py-1">
                    {item.children?.map((child) => (
                        ...
                        >
                            <div className={`w-1.5 h-1.5 rounded-full transition-all duration-500 ${isActive(child.href) ? 'bg-blue-400 scale-150 shadow-[0_0_8px_rgba(96,165,250,0.8)]' : 'bg-white/10 group-hover/sub:bg-white/40'}`} />
