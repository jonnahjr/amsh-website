...
export default function DepartmentCategoriesAdmin() {
    ...
    const handleDelete = async (id: string) => {
        if (!confirm('Are you sure you want to delete this department category?')) return;
        try {
    ...
    return (
        <div className="space-y-8 animate-fade-in">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h2 className="text-2xl font-black text-gray-900">Department Categories</h2>
                    <p className="text-gray-500 text-sm">Manage major clinical and administrative groupings (Department Boxes).</p>
                </div>
                <button
                        ...
                        .map((category) => (
                            <div key={category.id} className="bg-white rounded-[32px] border border-gray-50 shadow-sm hover:shadow-xl transition-all group overflow-hidden flex flex-col">
                                <div className="h-40 relative group-hover:scale-105 transition-transform duration-700">
                                    ...
                                    <div className="absolute bottom-4 left-4 right-4 text-white font-black text-lg drop-shadow-md">
                                        {category.name}
                                    </div>
                                </div>
                                <div className="p-6 flex-1 flex flex-col">
                                    <p className="text-gray-400 text-xs line-clamp-3 mb-6 flex-1">{category.description || 'No description provided.'}</p>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm animate-fade-in">
                    <div className="bg-white w-full max-w-2xl rounded-[40px] shadow-2xl overflow-hidden animate-slide-up">
                        <div className="px-8 py-6 bg-gray-50 border-b border-gray-100 flex items-center justify-between">
                            <h3 className="text-xl font-black text-gray-900">{editingCategory ? 'Edit' : 'Add'} Department Category</h3>
                        ...
                        <form onSubmit={handleSubmit} className="p-8 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
                            <div className="grid grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    ...
                                    <input
                                        ...
                                        placeholder="e.g. Clinical Departments"
                                    />
                                </div>
                                <div className="space-y-2">
                            ...
                            <div className="space-y-2">
                                <label className="text-[10px] font-black uppercase tracking-widest text-gray-400 ml-1">Visual Gradient (Classes)</label>
                                <input
                                    type="text"
                                    value={formData.gradient}
                                    onChange={(e) => setFormData({ ...formData, gradient: e.target.value })}
                                    className="w-full px-5 py-4 bg-gray-50 border-0 rounded-2xl focus:ring-2 focus:ring-blue-900/10 text-sm font-bold"
                                    placeholder="from-blue-900 to-blue-700"
                                />
                            </div>
                            ...
                            <div className="space-y-4 pt-4 border-t border-gray-100">
                                <div className="flex items-center justify-between">
                                    <label className="text-[10px] font-black uppercase tracking-widest text-gray-400 ml-1">Category Photo Backdrop</label>
