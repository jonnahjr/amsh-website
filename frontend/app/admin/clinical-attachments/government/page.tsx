...
export default function GovernmentAttachmentsAdmin() {
    ...
    const fetchSubmissions = async () => {
        ...
        try {
            ...
            // Filter only GOVERNMENT category
            const govSubmissions = (res.data.submissions || []).filter((sub: any) => {
                try {
                    const data = JSON.parse(sub.data);
                    return data.category === 'GOVERNMENT';
                } catch (e) {
                    return false;
                }
            });
            setSubmissions(govSubmissions);
        } catch (error) {
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h2 className="text-2xl font-black text-gray-900 flex items-center gap-3">
                        <BuildingLibraryIcon className="w-8 h-8 text-blue-900" />
                        Public Institution Attachments
                    </h2>
            ...
            <div className="grid grid-cols-1 gap-4">
                {loading ? (
                    ...
                    <div className="bg-white p-20 rounded-[40px] border border-gray-50 text-center">
                        ...
                        <p className="text-gray-400 font-bold uppercase tracking-widest text-sm">No government applications found</p>
                    </div>
                ) : (
                    filtered.map((sub) => {
                        const data = JSON.parse(sub.data);
                            ...
                            <div key={sub.id} className="bg-white p-6 md:p-8 rounded-[32px] border border-gray-50 shadow-sm hover:shadow-md transition-all group">
                                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                                    <div className="flex items-start gap-4">
                                        <div className="w-14 h-14 rounded-2xl bg-blue-50 flex items-center justify-center text-blue-900 flex-shrink-0 group-hover:scale-110 transition-transform">
