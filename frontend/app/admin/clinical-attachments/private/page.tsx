...
export default function PrivateAttachmentsAdmin() {
    ...
    const fetchSubmissions = async () => {
        ...
        try {
            ...
            // Filter only PRIVATE category
            const privateSubmissions = (res.data.submissions || []).filter((sub: any) => {
                try {
                    const data = JSON.parse(sub.data);
                    return data.category === 'PRIVATE';
                } catch (e) {
                    return false;
                }
            });
            setSubmissions(privateSubmissions);
        } catch (error) {
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h2 className="text-2xl font-black text-gray-900 flex items-center gap-3">
                        ...
                        Private College Attachments
                    </h2>
                    <p className="text-gray-500 text-sm">Manage applications from private medical and health science colleges.</p>
                </div>
                <div className="flex items-center gap-2">
                    <button 
                        onClick={() => exportToCSV(filtered, 'Private_Attachments_Export')}
            ...
            <div className="grid grid-cols-1 gap-4">
                {loading ? (
                    ...
                    <div className="bg-white p-20 rounded-[40px] border border-gray-50 text-center">
                        <AcademicCapIcon className="w-16 h-16 text-gray-100 mx-auto mb-4" />
                        <p className="text-gray-400 font-bold uppercase tracking-widest text-sm">No private applications found</p>
                    </div>
                ) : (
                    filtered.map((sub) => {
                        const data = JSON.parse(sub.data);
                            ...
                            <div key={sub.id} className="bg-white p-6 md:p-8 rounded-[32px] border border-gray-50 shadow-sm hover:shadow-md transition-all group">
                                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                                    <div className="flex items-start gap-4">
                                        <div className="w-14 h-14 rounded-2xl bg-amber-50 flex items-center justify-center text-amber-900 flex-shrink-0 group-hover:scale-110 transition-transform">
                                        ...
                                        <div>
                                            ...
                                            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-[11px] font-bold text-gray-400">
                                                ...
                                                <span className="text-blue-900 uppercase">{data.profession} ({data.studentCount} Students)</span>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-2">
